"""
Main Window Module
Contains the Tkinter GUI for the DSP Signal Processing application.
"""
...
from core.signal_io import ReadSignalFile, ReadFreqSignalFile, read_samples
from core.transformations import SignalComapreAmplitude, SignalComaprePhaseShift
...
def moving_avg_signal():
    ...
    if combo_conv.get() == 'MovingAvg_input':
        if window_size == 3:
            if CheckSignalsEquality("data/moving_avg/MovingAvg_out1.txt", new_indicies, new_samples):
                print("Moving Average W = 3 Test case passed successfully")
            else:
                print("Moving Average W = 3 Test case Failed")
        elif window_size == 5:
            if CheckSignalsEquality("data/moving_avg/MovingAvg_out2.txt", new_indicies, new_samples):
...
def first_deriv_signal():
    ...
    if combo_conv.get() == 'Derivative_input':
        if CheckSignalsEquality("data/derivative/1st_derivative_out.txt", new_indicies, new_samples):
...
def second_deriv_signal():
    ...
    plot_signal(ax_conv, new_indicies, new_samples, title=f"2nd Derivative of {combo_conv.get()}",
                canvas_obj=canvas_conv)
    if combo_conv.get() == 'Derivative_input':
        if CheckSignalsEquality("data/derivative/2nd_derivative_out.txt", new_indicies, new_samples):
...
def convolute_signals():
    ...
    if combo1_conv.get() == 'Signal 1' and combo2_conv.get() == 'Signal 2':
        if CheckSignalsEquality("data/convolution/Conv_output.txt", new_indicies, new_samples):
...
def fourier_transform():
    ...
    if combo_freq.get() == "Input_Signal_DFT":
        expected_amplitudes, expected_phases = ReadFreqSignalFile("data/dft/Output_Signal_DFT.txt")
...
def inverse_fourier_transform():
    ...
    if combo1_freq.get() == "Input_Signal_IDFT":
        if CheckSignalsEquality("data/dft/Output_Signal_IDFT.txt", indicies, samples):
...
def correlate_signals():
    ...
    if combo1_corr.get() == "Corr_input signal1" and combo2_corr.get() == "Corr_input signal2" and isNormalized.get():
        Compare_Signals("data/correlation/CorrOutput.txt", corr_indicies, corr_samples)
...
def generate_filter():
    ...
    if stop_anttenuation <= 21 :
        ...
        messagebox.showerror("Error","Stop attenuation value not supported")
        return
...
def apply_filter():
    ...
    Compare_Signals("data/fir/Testcase 2/ecg_low_pass_filtered.txt", conv_indicies, conv_samples)
...
# Default Settings - Load classification classes from new data directory
load_class("data/classification/Class1")
load_class("data/classification/Class2")
